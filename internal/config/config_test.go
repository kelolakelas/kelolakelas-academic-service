...
func TestLoadConfig(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			viper.Reset()
			t.Chdir(t.TempDir())
			for _, key := range []string{"DATABASE_URL", "DB_HOST", "DB_PORT", "DB_SSLMODE", "DB_CHANNEL_BINDING", "DB_USER", "DB_PASSWORD", "DB_NAME", "IDENTITY_GRPC_HOST", "BILLING_SERVICE_URL", "PORT", "JWT_SECRET"} {
				t.Setenv(key, "")
			}
			if tt.setup != nil {
				tt.setup(t)
