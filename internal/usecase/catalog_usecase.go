...
// CatalogTTL so a zero value cannot silently disable snapshot refresh.
func NewCatalogUsecase(repo domain.CatalogRepository, tenantClient grpcclient.TenantClient, ttl time.Duration) CatalogUsecase {
	if ttl <= 0 {
		ttl = CatalogTTL
	}
	return &catalogUsecase{repo: repo, tenantClient: tenantClient, ttl: ttl}
}
